...
public class ReduceHashTable<T> extends AbstractMutableHashTable<T> {
	...
	private static int hash(int code) {
		// Note: We don't need this, because the only problem that this could fix is too many
		// hash values nearby, which doesn't hurt us. (It could hurt open addressing with linear
		// probing, for example.) (If too many records hash to the same value, that can't be helped
		// by this anyway.)
		//
		// But:
		// Uncomment this when using HashTablePerformanceComparison to compare performance, because
		// CompactingHashTable has this code, and adjacent records generated by UniformIntPairGenerator
		// have adjacent keys, which greatly affects cache misses.
